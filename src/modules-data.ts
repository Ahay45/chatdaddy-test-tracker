...
const STATIC: Record<string, StaticConfig> = {
  ...
  'error-scenarios': {
    ...
    subFeatures: [
      ...
      { name: '404 error page — "ERROR 404" title + localized message + Go Back button', done: false },
      { name: '404 error page — illustration / image', done: false },
      { name: 'Runtime crash page — "Oops!" title + localized message', done: false },
      { name: 'Runtime crash page — Submit Issue Report button (Paperform bug report)', done: false },
      { name: 'Runtime crash page — Refresh button', done: false },
      { name: 'Chunk loading error — auto-reload on "Failed to fetch dynamically imported module"', done: false },
      { name: 'Chunk loading error — max reload attempts guard (no infinite loop)', done: false },
      ...
      { name: 'Feature-level error boundary — inline retry UI per section (not full page crash)', done: false },

      // ── 2. Empty States ───────────────────────────────────────────────────
      { name: 'Empty state — "No {items} Exist" with create CTA', done: false },
      { name: 'Empty state — "No {items} Found" after filter/search', done: false },
      { name: 'Empty state — error on initial load with Retry button', done: false },
      { name: 'Empty state — no chats in inbox', done: false },
      { name: 'Empty state — no channels connected', done: false },
      { name: 'Empty state — no contacts in CRM', done: false },
      { name: 'Empty state — no flows in Flow Builder', done: false },
      { name: 'Empty state — no broadcasts', done: false },
      { name: 'Empty state — no notifications/webhooks', done: false },
      { name: 'Empty state — no orders', done: false },
      { name: 'Empty state — no products', done: false },
      ...
      { name: 'Error message parser — "Network Error" → friendly message', done: false },
      { name: 'Error message parser — "Failed to fetch" → friendly message', done: false },
      { name: 'Error message parser — timeout → "taking too long" message', done: false },
      { name: 'Error message parser — 400 invalid file format message', done: false },
      { name: 'Error message parser — 402 "Not enough credits" + top-up CTA', done: false },
      { name: 'Error message parser — 403 "Insufficient Access" + missing scopes list', done: false },
      { name: 'Error message parser — 404 upload endpoint not found message', done: false },
      { name: 'Error message parser — 413 file too large with max size in message', done: false },
      { name: 'Error message parser — 415 unsupported file type message', done: false },
      { name: 'Error message parser — 429 too many requests message', done: false },
      { name: 'Error message parser — 500+ server error with status code', done: false },
      { name: 'Error message parser — 504 overloaded / try again message', done: false },
      { name: 'Error message parser — Meta server issue → human-readable message', done: false },
      { name: 'RetryUI component — shows parsed error + Retry button inline', done: false },
      { name: 'RetryUI component — template error detected → "Go to Message Flow" button', done: false },

      // ── 4. Form Validation Errors ─────────────────────────────────────────
      { name: 'Inline field validation — required field highlight + message', done: false },
      { name: 'Validation — "Name cannot be empty"', done: false },
      { name: 'Validation — "Invalid phone number format"', done: false },
      { name: 'Validation — "Enter a valid email address"', done: false },
      { name: 'Validation — "Enter a valid website address"', done: false },
      ...
      // ── 5. Auth Errors ────────────────────────────────────────────────────
      { name: 'Auth — phone already registered → "Try logging in instead"', done: false },
      { name: 'Auth — number not on WhatsApp message', done: false },
      { name: 'Auth — "No internet connection" offline error', done: false },
      { name: 'Auth — OTP send failed generic toast', done: false },
      { name: 'Auth — OTP: "Please enter the complete 6-digit code"', done: false },
      { name: 'Auth — OTP: "❌ Invalid verification code"', done: false },
      { name: 'Auth — OTP: "⏰ Verification code expired"', done: false },
      { name: 'Auth — OTP: "🚫 Too many verification attempts — session locked"', done: false },
      { name: 'Auth — OTP: "Verification session not found — restart process"', done: false },
      { name: 'Auth — OTP resend cooldown: "Please wait 60 seconds"', done: false },
      { name: 'Auth — OTP: 429 rate limit → wait 5 minutes message', done: false },
      { name: 'Auth — OTP: 403 verification blocked → contact support', done: false },
      { name: 'Auth — password reset error display', done: false },
      { name: 'Auth — 401 authentication failed message', done: false },

      // ── 6. WebSocket / Connection Errors ──────────────────────────────────
      { name: 'Connection broken snackbar — "Lost live connection" alert', done: false },
      { name: 'Connection broken — "Data may be stale" message', done: false },
      { name: 'Connection broken — Refresh button to reload', done: false },
      { name: 'WebSocket reconnecting state — spinner / connecting indicator', done: false },
      { name: 'WebSocket JSON parse error — silent recovery (no crash)', done: false },

      // ── 7. File Upload Errors ─────────────────────────────────────────────
      { name: 'Upload — file too large toast with max MB in message', done: false },
      { name: 'Upload — unsupported file type toast', done: false },
      { name: 'Upload — "Some files failed to upload" multi-file error', done: false },
      { name: 'Upload — max 10 files per message enforcement', done: false },
      { name: 'Upload — failed attachment state with retry option in compose', done: false },
      ...
      { name: 'Message send failed — error icon on bubble + retry button', done: false },
      ...
      { name: 'Toast system — error variant (red)', done: false },
      { name: 'Toast system — success variant (green)', done: false },
      { name: 'Toast system — warning variant (yellow)', done: false },
      { name: 'Toast system — configurable duration (3000 / 4000 / 5000 / 6000 / 7000ms)', done: false },
      { name: 'Toast system — auto-dismiss', done: false },
    ],
